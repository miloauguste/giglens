...
### Account System
- **Email + password registration** — no social login in MVP (reduces dependency on third-party SDKs)
...
- **Session management:** JWT tokens with expiration, stored in Android EncryptedSharedPreferences

### On-Device Storage Security
- **User credentials are NEVER stored in plain text.**
- **EncryptedSharedPreferences** (AndroidX Security library) for storing:
  - Session token (JWT)
  - User email (for display only)
  - Account preferences
- **Android Keystore** backs the encryption — keys are hardware-protected on supported devices
- **Room database encryption** via SQLCipher when user data is present:
  - Offer data linked to user ID
  - Encryption key derived from Android Keystore, not user password
- **Password is NEVER stored locally** — only sent to server during auth, then discarded
- **Biometric unlock** (fingerprint/face) as optional convenience for returning users (Phase 3)

### Backend Auth Requirements (when server is built)
- **Passwords hashed server-side** with bcrypt (cost factor 12+)
- **HTTPS only** — all auth endpoints over TLS 1.3
- **Rate limiting** on login/register endpoints (5 attempts per minute)
- **JWT tokens:**
  - Access token: 15-minute expiry
  - Refresh token: 30-day expiry, stored in EncryptedSharedPreferences
  - Refresh rotation — each use issues a new refresh token and invalidates the old one
- **Password reset** via email with time-limited token (1 hour expiry)
- **Account deletion** endpoint — GDPR/CCPA compliance, removes all user data
...
[200~cat >> /home/poppa/giglens/CLAUDE.md << 'SECTION'
...
1. **NEVER log user email, password, or tokens** — not even in debug builds.
2. **NEVER store password locally** — authenticate against server, store only JWT.
3. **Wipe credentials on logout** — clear EncryptedSharedPreferences and re-encrypt DB if needed.
4. **Offline-first:** App must function without auth for share-to-capture. Auth required only for analytics sync, trial validation, and Pro features.
5. **Graceful degradation:** If token expires and server is unreachable, allow continued local capture. Sync when reconnected.
6. **No third-party auth SDKs** (Google Sign-In, Facebook Login) in MVP — reduces attack surface and dependency risk.
